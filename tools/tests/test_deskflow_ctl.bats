...
# pid launchd reports, and ps.txt is the process table (pid uid path).
...
case "$verb" in
  ...
  bootstrap)
    ...
    if [[ -f "$SHIM_STATE/autostart/$label" ]]; then
      ...
      echo "$(cat "$SHIM_STATE/pid/$label")	501	$(cat "$SHIM_STATE/autostart/$label.path")" >> "$SHIM_STATE/ps.txt"
    fi
    ;;
  bootout)
    label="${1##*/}"
...
@test "status inventories agents and processes by executable path" {
  ...
  [[ "$output" == *"300 501 $APP/Contents/MacOS/deskflow-core yes"* ]]
  [[ "$output" == *"777 501 /tmp/build/bin/deskflow-core NO"* ]]
}
