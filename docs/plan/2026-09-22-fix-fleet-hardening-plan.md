...
- **K10 follow-up (adversarial review, 2026-09-25)**: the macbookpro converge tick had exited 126 since install (TCC EPERM on the `~/Desktop` checkout path) -- it never ran, so it never spawned a duplicate core; K10's "converge spawned duplicates on macbookpro" narrative is withdrawn (its process-table guard closes a latent path). The one launcher in the tree that starts a core with no process-table check is the GUI's own `kickstartExternalCore` (`src/lib/gui/core/CoreProcess.cpp:293-313`: `launchctl kickstart -k` on the core label). Follow-up (C++, not in K10): consult the process table there, or route it through `deskflow-ctl restart`, which refuses beside a stray.
