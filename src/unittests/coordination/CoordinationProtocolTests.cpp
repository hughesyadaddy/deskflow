...
void CoordinationProtocolTests::legacyMessagesDecodeForDrop()
{
  // Legacy mesh v1 messages are never sent, but must stay classifiable so
  // the transport can identify and drop them (CoordinationMesh::handleClient).
  const auto cursor = protocol::decode(R"({"t":"cursor","host":"macbook","seq":9,"token":"secret"})");
  QCOMPARE(cursor.type, Message::Type::Cursor);

  const auto keyFwd = protocol::decode(R"({"t":"keyfwd","from":"windows-pc","phase":"down","id":7,"mask":8})");
  QCOMPARE(keyFwd.type, Message::Type::KeyFwd);
}
