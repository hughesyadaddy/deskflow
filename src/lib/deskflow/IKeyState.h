...
public:
  ...
  */
  ...
  //! Set a lock modifier to an absolute state
  /*!
  Drives the OS lock state for exactly one of \c KeyModifierCapsLock,
  \c KeyModifierNumLock or \c KeyModifierScrollLock to \p on. Lock keys are
  STATE, not toggles: the server ships the primary's lock mask on every
  enter and on every change, and the target applies it absolutely, so a
  half-duplex Caps key can never drift out of phase with the primary. The
  default does nothing; platforms override with their lock-state primitive
  (macOS: level-triggered alphaShift, Windows: keybd_event pair).
  */
  virtual void setToggleState(KeyModifierMask toggle, bool on)
  {
    (void)toggle;
    (void)on;
  }

  //! Release keys this process injected that the OS still reports down
  /*!
  Called on a structural boundary (primary teardown, role flip) after
  fakeAllKeysUp(). Platforms that can enumerate their own injected key
  state override this to release anything the shadow ledger lost track
  of. The default does nothing.
  */
