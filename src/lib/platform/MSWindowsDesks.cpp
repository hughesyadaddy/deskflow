...
// PowerToys Keyboard Manager's HandleShortcutRemapEvent -- the state machine
// that, once a Win-shortcut remap is "invoked", treats any further key as
// case 5 ("another key while invoked") and re-presses the original Win
// modifier -- gates on exact equality against its own sentinel,
// KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG = 0x101
// (KeyboardManagerConstants.h), meaning "this key was generated by my own
// shortcut remap, not new input". A single-bit tag (dwExtraInfo & 1,
// CommonSharedConstants::KEYBOARDMANAGER_INJECTED_FLAG) only gates KBM's
// single-key remap paths, NOT this one (confirmed against current
// microsoft/PowerToys source, review 2026-09-25) -- a first attempt tagged
// with just that bit compiled and shipped inert against the actual bug.
// 0x101 already has bit 0 set, so it satisfies both checks with one value.
// Our audit/boundary releases are Deskflow cleaning up ITS OWN stuck
// modifier, not a new user keypress, so tagging them with this sentinel
// stops KBM from reading our release (or the menu-mask dummy key next to
// it) as "another key while a Win-shortcut is invoked" and re-pressing Win
// right back down -- the self-inflicted loop diagnosed 2026-09-25 (tiny11:
// audit released LWin, KBM re-pressed it within ~2 s, repeating every audit
// tick; the configured remap is a shortcut remap, e.g. Win+R->Ctrl+R).
// Deliberately NOT used by send_keyboard_input above: ordinary relayed keys
// must still look like an ordinary press so real Win-shortcut remaps keep
// working during normal use.
constexpr ULONG_PTR kSkipThirdPartyRemapFlag = 0x101;
...
void deskSanitizeStaleModifiers(MSWindowsDesks::StaleModifierAudit *audit, const std::wstring &deskName)
{
  ...
  if (maskMenu) {
    ...
    dummy[0].ki.dwExtraInfo = kSkipThirdPartyRemapFlag;
    dummy[1].type = INPUT_KEYBOARD;
    dummy[1].ki.wVk = 0xE8;
    dummy[1].ki.dwFlags = KEYEVENTF_KEYUP;
    dummy[1].ki.dwExtraInfo = kSkipThirdPartyRemapFlag;
